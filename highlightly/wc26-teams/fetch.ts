...
const LIMIT = 500; // max page size for /teams
const TEAM_TYPE = "national";
const OUTPUT_FILE = path.join(__dirname, "data", "national-teams.json");
const PROGRESS_FILE = path.join(
  __dirname,
  "data",
  "national-teams-progress.json",
);
...
async function loadExistingTeams(): Promise<Team[]> {
  try {
...
async function saveTeams(teams: Team[]): Promise<void> {
  ...
  await fs.writeFile(OUTPUT_FILE, JSON.stringify(teams, null, 2));
}

async function fetchPage(offset: number, apiKey: string): Promise<ApiResponse> {
  const url = `${BASE_URL}/teams?type=${TEAM_TYPE}&limit=${LIMIT}&offset=${offset}`;
...
async function main(): Promise<void> {
  ...
  console.log(
    `Starting from offset ${offset}${totalCount ? ` / ${totalCount}` : ""}`,
  );
  ...
  while (totalCount === null || offset < totalCount) {
    try {
      ...
      if (response.data.length === 0) {
        console.log(`No more teams at offset ${offset}. Done.`);
        break;
      }
      ...
      if ((err as Error).message === "QUOTA_EXHAUSTED" && keyIndex === 0) {
        console.warn(
          `Key 1 exhausted at offset ${offset}. Switching to key 2.`,
        );
        keyIndex = 1;
        continue;
      }
      ...
      console.log(
        `RESUME FROM OFFSET: ${offset} (${teams.length} teams saved so far)`,
      );
      process.exitCode = 1;
      return;
    }
  ...
  console.log(
    `✅ Done. ${teams.length} national teams saved to ${OUTPUT_FILE}`,
  );
}
